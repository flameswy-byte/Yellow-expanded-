...
INWARD = {'up': (0, 1), 'down': (0, -1), 'left': (1, 0), 'right': (-1, 0)}
# which way a gate shoves the player: back into the map, away from that edge
PUSH = {'left': 'right', 'right': 'left', 'up': 'down', 'down': 'up'}
MARK = '@ --- open hoenn: gates generated by tools/newmaps.py ---'
MARK_END = '@ --- end open hoenn gates ---'
HOOK = '@ open hoenn'      # tags the one line spliced into a vanilla script
...
def edge_cells(L, side):
    """where to stand a gate trigger on one edge, one cell in from the rim.

    Not on the rim itself. The rim is the cell the player is standing on while
    the game is already loading the map next door - LoadMapFromCameraTransition
    swaps gMapHeader, the object events and the temp vars out from under
    whatever is running - and a coord event there fires into that. Vanilla puts
    its own "I need a POKeMON" trigger at Littleroot (10,1), one row inside the
    northern rim at y=0, for the same reason.

    Three cells matter, in a line running inwards: the rim (which the player
    has to be able to stand on, or this is not an exit at all), the trigger
    cell, and the cell behind it, which is where the script pushes the player
    back to and therefore has to be somewhere they can be.
    """
    w, h = L['width'], L['height']
    blk = open(f'{ROOT}/{L["blockdata_filepath"]}', 'rb').read()
    ...
    def cell(x, y):
        o = (y * w + x) * 2
        if not (0 <= x < w and 0 <= y < h) or o + 1 >= len(blk):
            return None
        v = blk[o] | (blk[o+1] << 8)
        return (((v >> 10) & 3) == 0, (v >> 12) & 0xF)
    ...
    for a in span:
        rx, ry = ((a, 0) if side == 'up' else (a, h-1) if side == 'down' else
                  (0, a) if side == 'left' else (w-1, a))
        tx, ty = rx + dx, ry + dy
        # a corner is one cell in from this edge but still sitting on the next
        # one along, which is a rim like any other and just as likely to have a
        # connection across it. Keep every trigger off the boundary entirely.
        if not (0 < tx < w - 1 and 0 < ty < h - 1):
            continue
        three = [cell(rx + dx*i, ry + dy*i) for i in range(3)]
        if any(c is None or not c[0] for c in three):
            continue
        e = [c[1] for c in three]
        if not all(e[i] == e[i+1] or 0 in (e[i], e[i+1]) or 15 in (e[i], e[i+1])
                   for i in (0, 1)):
            continue
        out.append((tx, ty, PUSH[side]))
    return out


def push_from(L, x, y, side):
    """which way to shove a player standing here, or None if nowhere is safe.

    Inward first, then anything else that is somewhere they can stand. Falling
    off the end means the cell has no safe push and no trigger belongs on it -
    better an exit that is not gated than one that walks you into a tree.
    """
    w, h = L['width'], L['height']
    blk = open(f'{ROOT}/{L["blockdata_filepath"]}', 'rb').read()
    ...
    def at(cx, cy):
        if not (0 <= cx < w and 0 <= cy < h):
            return None
        v = blk[(cy * w + cx) * 2] | (blk[(cy * w + cx) * 2 + 1] << 8)
        return (((v >> 10) & 3) == 0, (v >> 12) & 0xF)

    here = at(x, y)
    order = [PUSH[side]] + [d for d in ('up', 'down', 'left', 'right')
                            if d != PUSH[side]]
    for d in order:
        ddx, ddy = STEP[d]
        c = at(x + ddx, y + ddy)
        if c is None or not c[0]:
            continue
        a, b = here[1], c[1]
        if a == b or 0 in (a, b) or 15 in (a, b):
            return d
    return None

def gate_leaks(L, sides, trig):
    """rim cells a player can reach without stepping on a trigger first.

    Moving a trigger one cell inwards is only safe if the ring it makes is
    closed. Where the cell behind a trigger is a wall the trigger is dropped,
    and that gap has to be one the player cannot walk around to - so walk it.
    """
    w, h = L['width'], L['height']
    blk = open(f'{ROOT}/{L["blockdata_filepath"]}', 'rb').read()
    cells = [(blk[i*2] | (blk[i*2+1] << 8)) for i in range(w * h)]
    walk = lambda x, y: ((cells[y*w + x] >> 10) & 3) == 0
    elev = lambda x, y: (cells[y*w + x] >> 12) & 0xF
    ...
    for side in sides:
        ...
        span = range(w) if side in ('up', 'down') else range(h)
        for a in span:
            r = ((a, 0) if side == 'up' else (a, h-1) if side == 'down' else
                 (0, a) if side == 'left' else (w-1, a))
            rim.add(r)
            band |= {r, (r[0] + dx, r[1] + dy)}

    # start from the town proper - the rim and the ring the triggers sit on are
    # the thing being tested, so nothing in them counts as somewhere the player
    # already is
    start = [(x, y) for y in range(h) for x in range(w)
             if walk(x, y) and (x, y) not in band]
    ...
    while stack:
        ...
        for nx, ny in ((x+1, y), (x-1, y), (x, y+1), (x, y-1)):
            if not (0 <= nx < w and 0 <= ny < h) or (nx, ny) in seen:
                continue
            if not walk(nx, ny) or (nx, ny) in trig:
                continue
            a, b = elev(x, y), elev(nx, ny)
            if not (a == b or 0 in (a, b) or 15 in (a, b)):
                continue
            seen.add((nx, ny))
            stack.append((nx, ny))
    return sorted(seen & rim)


def guard_script(name, g, push):
    """the block appended to a map's scripts.inc for one push-back direction.

    applymovement does not consult collision - a scripted walk goes wherever it
    is pointed - so a gate that always shoves the player the same way will
    eventually shove one into a tree. The direction is chosen per trigger by
    looking at what is actually next to it, and this generates one script per
    direction that turned out to be needed.
    """
    S, cap = [], push.capitalize()
    ...
    if g['kind'] == GUARD_PARTY:
        # Belt and braces: _OpenHoennGateSetup should already have taken
        # VAR_TEMP_2 off zero and left the coord events unmatched, so getting
        # here at all means the on-transition script did not run.
...
def gates(dry):
    ...
    for const, g in GUARDS.items():
        ...
        body = []
        if g['kind'] == GUARD_PARTY:
            # A coord event only fires while its var reads the value it was
            # given, so the cheapest way to switch these off for good is to
            # move the var. VAR_TEMP_2 is zeroed by ClearTempFieldEventData on
            # every map load, and RunOnTransitionMapScript runs after it -
            # both on a warp and on walking in from a connection - so this is
            # the one place the answer can be recomputed. Without it VAR_TEMP_2
            # is zero forever, every edge tile runs a script on every step, and
            # TryStartStepBasedScript returns before it can get to warps, the
            # step counter or the repel counter.
            body += [f'{name}_EventScript_OpenHoennGateSetup::',
                     f'\tgoto_if_set FLAG_SYS_POKEMON_GET, '
                     f'{name}_EventScript_OpenHoennGateOff',
                     '\treturn',
                     '',
                     f'{name}_EventScript_OpenHoennGateOff::',
                     '\tsetvar VAR_TEMP_2, 1',
                     '\treturn',
                     '',
                     f'{name}_EventScript_OpenHoennGateOpen::', '\tend', '']
        used = set()

        def trigger(x, y, push):
            used.add(push)
            return {'type': 'trigger', 'x': x, 'y': y, 'elevation': 0,
                    'var': 'VAR_TEMP_2' if g['kind'] == GUARD_PARTY else g['var'],
                    'var_value': '0' if g['kind'] == GUARD_PARTY else g['value'],
                    'script': f'{name}_EventScript_OpenHoennGate{push.capitalize()}'}

        events = []
        for side in g['sides']:
            events += [trigger(x, y, p) for x, y, p in edge_cells(L, side)]

        # A corner is reachable along the boundary from an edge nobody is
        # gating - Littleroot's top row is open grass all the way to (0,0), and
        # (0,0) is on the left rim - so no ring drawn inside the map can close
        # there. Those few cells get a trigger on the rim after all. It is the
        # thing being avoided, but only where the alternative is a hole, and
        # only ever for a player who has no POKeMON to walk out with.
        # One at a time: rim cells are all neighbours of each other, so as soon
        # as the player can reach any of them the rest of that run is reachable
        # too and the whole rim looks like a leak. Plugging the first one and
        # asking again narrows it down to the handful of cells that are really
        # the way in.
        placed = {(e['x'], e['y']) for e in events}
        giveup = set()
        for _ in range(200):
            leaks = [c for c in gate_leaks(L, g['sides'], placed)
                     if c not in giveup]
            if not leaks:
                break
            x, y = leaks[0]
            side = next(s for s in g['sides'] if
                        (s == 'left' and x == 0) or
                        (s == 'right' and x == L['width'] - 1) or
                        (s == 'up' and y == 0) or
                        (s == 'down' and y == L['height'] - 1))
            push = push_from(L, x, y, side)
            if push is None:
                giveup.add((x, y))
                continue
            events.append(trigger(x, y, push))
            placed.add((x, y))
        # only the push-back directions that turned out to be needed
        for push in sorted(used):
            body += guard_script(name, g, push)
        ...
        # and call the setup from the map's own on-transition script
        src = '\n'.join(l for l in src.split('\n') if HOOK not in l)
        if g['kind'] == GUARD_PARTY:
            label = f'{name}_OnTransition:'
            if label + '\n' not in src:
                sys.exit(f'{name} has no {label} to hang the gate setup on')
            i = src.index(label + '\n') + len(label) + 1
            src = (src[:i] + f'\tcall {name}_EventScript_OpenHoennGateSetup'
                   f'\t{HOOK}\n' + src[i:])
        ...
        leaks = gate_leaks(L, g['sides'], placed)
        rim = sum(1 for e in events if e['x'] in (0, L['width'] - 1)
                  or e['y'] in (0, L['height'] - 1))
        print(f'  gate {name:16s} {len(events)} triggers on {", ".join(g["sides"])}'
              f'   ({g["kind"]}, {rim} on the rim, pushes '
              f'{"/".join(sorted(used))})'
              + (f'   UNGATED: {leaks[:6]}' if leaks else ''))
        if not dry:
            open(sp, 'w').write(src)
...
def main():
    ...
    ap.add_argument('--gates-only', action='store_true',
                    help='redo just the town-exit gates, touching no map data')
    a = ap.parse_args()
    dry = a.dry_run
